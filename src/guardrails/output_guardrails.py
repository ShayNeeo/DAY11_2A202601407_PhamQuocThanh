...
def content_filter(response: str) -> dict:
    ...
    PII_PATTERNS = {
        ...
        "DB Host": r"db\.vinbank\.internal(:5432)?",
    }
...
class OutputGuardrailPlugin(base_plugin.BasePlugin):
    ...
    async def after_model_callback(
        ...
        *,
        ...
        current_text = filter_res["redacted"]
        if not filter_res["safe"]:
            self.redacted_count += 1

        # 2. If use_llm_judge: call llm_safety_check(current_text)
        if self.use_llm_judge:
            judge_res = await llm_safety_check(current_text)
            if not judge_res["safe"]:
                self.blocked_count += 1
                current_text = "i cannot share internal system details"

        # 3. Return llm_response (possibly modified)
        if current_text != response_text and hasattr(llm_response, "content"):
