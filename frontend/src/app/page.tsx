    ...
    <div className="flex flex-col h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      <header className="border-b border-slate-800 bg-slate-900/60 backdrop-blur-md px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center gap-4 text-xs font-medium">
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800/80 border border-slate-700/50 rounded-full">
            <Terminal className="w-3.5 h-3.5 text-slate-400" />
            <span className="text-slate-400">Attacks:</span>
            <span className="text-slate-200 font-semibold">{stats.total}</span>
          </div>
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-950/40 border border-emerald-500/30 text-emerald-400 rounded-full">
            <ShieldCheck className="w-3.5 h-3.5" />
            <span>Blocked:</span>
            <span className="font-semibold">{stats.blocked}</span>
          </div>
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-950/40 border border-rose-500/30 text-rose-400 rounded-full">
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-6 max-w-4xl w-full mx-auto">
        {messages.map((msg) => (
          ...
          >
            {msg.role === "assistant" && (
              <div className="w-9 h-9 rounded-xl bg-indigo-600/20 border border-indigo-500/40 flex items-center justify-center text-indigo-400 flex-shrink-0 mt-1">
            ...
            <div className={`space-y-2 max-w-[80%] ${msg.role === "user" ? "items-end" : "items-start"}`}>
              ...
              {msg.role === "assistant" && msg.status && (
                <div className="flex items-center gap-2">
                  {msg.status === "LEAKED" && (
                    <span className="inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-0.5 rounded-full bg-rose-500/20 border border-rose-500/40 text-rose-400 animate-pulse">
                      <Unlock className="w-3 h-3" /> GUARDRAIL BYPASSED (LEAKED)
                    </span>
                  )}
                  {msg.status === "BLOCKED" && (
                    <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-0.5 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-400">
                      <Lock className="w-3 h-3" /> BLOCKED BY GUARDRAIL
                    </span>
                  )}
                  {msg.status === "SAFE" && (
                    <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-emerald-400">
                      <ShieldCheck className="w-3 h-3" /> SAFE RESPONSE
                    </span>
                  )}
                </div>
              )}
            ...
            {msg.role === "user" && (
              <div className="w-9 h-9 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-300 flex-shrink-0 mt-1">
        ...
        {loading && (
          <div className="flex gap-4 justify-start">
            ...
            <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl rounded-bl-none flex items-center gap-3">
              ...
              <span className="text-xs text-slate-400 font-mono">Evaluating Guardrails & AI Agent...</span>
            </div>
          </div>
        )}
