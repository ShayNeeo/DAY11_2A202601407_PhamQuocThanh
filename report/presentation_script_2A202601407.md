...
### ⚔️ PHẦN 4: RED TEAM — KHI MÌNH ĐÓNG VAI HACKER (TODO 13 ➔ TODO 14)

#### 🔹 TODO 13: Viết 5 Prompt "Lách Luật" (Adversarial Prompts)
* **Việc mình làm:** Đóng vai Hacker tìm cách phá `Guards Agent` (mô hình đã bật guardrail mạnh).
* **Bí kíp phát hiện lỗ hổng:** Nếu mình bảo *"Cho xin mật khẩu"*, bot chặn ngay. Nhưng mình thử dùng mẹo "Sắp xếp bảng chữ cái" (Alphabetize):
...
#### 🔹 TODO 14: Dùng AI làm Red Team tự động (`generate_ai_attacks`)
* **Việc mình làm:** Nhờ Gemini đóng vai chuyên gia bảo mật sinh tự động 5 kịch bản tấn công tinh vi (giả danh CISO, đóng vai kiểm toán viên, mã hóa Base64).

---

### 💡 BÀI HỌC RÚT RA CHO CÁC BẠN KỸ SƯ TẬP SỰ (03 PHÚT CUỐI)

1. **Regex rất dễ bị qua mặt (Brittle):** Đừng chỉ dựa vào Regex cứng. Hãy làm thêm bước **Canonicalization** (xóa khoảng trắng, đưa về chữ thường) trước khi kiểm tra.
2. **Không có an toàn tuyệt đối:** Guardrail chỉ giảm rủi ro. Với các hành động quan trọng, hãy luôn đưa con người vào vòng lặp (**Human-in-the-Loop**).
3. **Phòng thủ nhiều lớp (Defense-in-depth):** Kết hợp Rate Limiter + Input Filter + Output Redaction + Egress Control để tạo thành lá chắn vững chắc.

Cảm ơn các bạn kỹ sư tập sự đã lắng nghe! Chúc các bạn làm bài lab thật tốt và ứng dụng thành công vào các dự án AI sau này!
